...
2. 确保 PHP 和 cURL 扩展已安装
```bash
php -v
php -m | grep curl
```

3. 配置 Web 服务器（Apache/Nginx）指向项目目录
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| msg | string | 是 | 搜索关键词（地区、展会名称等） |
| debug | string | 否 | 调试模式：`raw`（原始数据）/ `response`（原始响应） |
...
- 数据来源：[无差别同人站](https://www.allcpp.cn/)
- Koishi插件（自用的版本）：[anime-convention-lizard](https://github.com/lizard0126/anime-convention-lizard)
